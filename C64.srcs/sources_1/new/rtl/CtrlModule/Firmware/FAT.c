...
char DirEntryLFN[MAXDIRENTRIES][264];
...
char t_DirEntryLFN[MAXDIRENTRIES][264];
...
/*    switch (sector_buffer[450])
    ...
    if (fat32)
    {
        if (mstrncmp((const char*)&sector_buffer[0x52], "FAT32   ", 8) != 0) // check file system type
            return(0);
...
int FileOpen(fileTYPE *file, char *name)
{
    ...
    while (1)
    {
        for (iEntry = 0; iEntry < nEntries; iEntry++)
        {
            ...
            if (pEntry->Name[0] != SLOT_EMPTY && pEntry->Name[0] != SLOT_DELETED) // valid entry??
            {
                ...
                {
                    if (mstrncmp((const char*)pEntry->Name, name, sizeof(file->name)) == 0)
                    {
                        copyname(file->name, (const char*)pEntry->Name, sizeof(file->name));
    ...
    mmemset(file, 0, sizeof(fileTYPE));
    return(0);
}

int lfn_checksum(unsigned char *pName)
{
    unsigned char i = 11;
        ...
        for (iEntry = 0; iEntry < nEntries; iEntry++)
        {
            ...
            if (pEntry->Name[0] != SLOT_EMPTY && pEntry->Name[0] != SLOT_DELETED) // valid entry??
            {
                ...
                else // if not an LFN entry
                {
                    ...
                    {
                        if (extension[0] == '*' || mstrncmp((const char*)&pEntry->Name[8], extension, 3) == 0 || options & SCAN_DIR && pEntry->Attributes & ATTR_DIRECTORY)
                        {
                            if (mode == SCAN_INIT)
                            { // scan the directory table and return first MAXDIRENTRIES alphabetically sorted entries
                                if (nDirEntries < MAXDIRENTRIES) // initial directory scan (first 8 entries)
                                {
                                    ...
                                    if (lfn[0])
                                        if (lfn_checksum(pEntry->Name) == name_checksum)
                                            copyname(DirEntryLFN[nDirEntries], lfn, sizeof(lfn));
                                ...
                                else
                                {
                                    if (CompareDirEntries(pEntry, lfn, &DirEntry[sort_table[MAXDIRENTRIES-1]], DirEntryLFN[sort_table[MAXDIRENTRIES-1]]) < 0) // compare new entry with the last already found
                                    {
                                        ...
                                        if (lfn[0])
                                            if (lfn_checksum(pEntry->Name) == name_checksum)
                                                copyname(DirEntryLFN[sort_table[MAXDIRENTRIES-1]], lfn, sizeof(lfn));
                                    }
                                }
                            ...
                            { // find a dir entry with given cluster number and store it in the buffer
                                ...
                                { // directory entry found
                                    ...
                                    if (lfn[0])
                                        if (lfn_checksum(pEntry->Name) == name_checksum)
                                            copyname(DirEntryLFN[0], lfn, sizeof(lfn));
                            ...
                            else if (mode == SCAN_NEXT) // replace the last dir entry with the new (higher) one
                            {
                                if (nNewEntries == 0) // no entry higher than the last one has been found yet
                                {
                                    if (CompareDirEntries(pEntry, lfn, &DirEntry[sort_table[MAXDIRENTRIES-1]], DirEntryLFN[sort_table[MAXDIRENTRIES-1]]) > 0) // found entry higher than the last one
                                    {
                                        ...
                                        if (lfn[0])
                                            if (lfn_checksum(pEntry->Name) == name_checksum)
                                                copyname(DirEntryLFN[sort_table[0]], lfn, sizeof(lfn));
                            ...
                            else if (mode == SCAN_NEXT_PAGE) // find next 8 entries
                            {
                                if (CompareDirEntries(pEntry, lfn, &DirEntry[sort_table[MAXDIRENTRIES-1]], DirEntryLFN[sort_table[MAXDIRENTRIES-1]]) > 0) // compare with the last visible entry
                                {
                                    if (nNewEntries < MAXDIRENTRIES) // initial directory scan (first 8 entries)
                                    {
                                        ...
                                        if (lfn[0])
                                            if (lfn_checksum(pEntry->Name) == name_checksum)
                                                copyname(t_DirEntryLFN[nNewEntries], lfn, sizeof(lfn));
                                    ...
                                    else
                                    {
                                        if (CompareDirEntries(pEntry, lfn, &t_DirEntry[t_sort_table[MAXDIRENTRIES-1]], t_DirEntryLFN[t_sort_table[MAXDIRENTRIES-1]]) < 0) // compare new entry with the last already found
                                        {
                                            ...
                                            if (lfn[0])
                                                if (lfn_checksum(pEntry->Name) == name_checksum)
                                                    copyname(t_DirEntryLFN[t_sort_table[MAXDIRENTRIES-1]], lfn, sizeof(lfn));
                                        }
                                    }
...
int FileSeek(fileTYPE *file, unsigned long offset, unsigned long origin)
{
...
// origin can be set to SEEK_SET or SEEK_CUR
    ...
    { // different clusters, get next one
        ...
        if (sb != buffered_fat_index)
        {
            if (!sd_read_sector(fat_start + sb, (unsigned char*)&fat_buffer)) // read sector of FAT if not already in the buffer
                return(0);
...
int FileReadEx(fileTYPE *file, unsigned char *pBuffer, unsigned long nSize)
{
    unsigned long sb;
    ...
    while (nSize)
    {
        ...
        if (!sd_read_sectorMultiple(sb, pBuffer, bc))
            return 0;
...
int FileWrite(fileTYPE *file,unsigned char *pBuffer)
{
    ...
    return(sd_write_sector(sector, pBuffer)); // write sector from drive
}

int FileCreate(unsigned long iDirectory, fileTYPE *file)
{
    ...
    */
    ...
    while (1)
    {
        for (iEntry = 0; iEntry < nEntries; iEntry++)
        {
            if ((iEntry & 0x0F) == 0) // first entry in sector, load the sector
            {
                sd_read_sector(iDirectorySector++, sector_buffer); // read directory sector
                pEntry = (DIRENTRY*)sector_buffer;
            }
            else
                pEntry++;
                ...
                while (fat_index < fat_size)
                {
                    ...
                    { // search through all entries in current sector
                        ...
                        {   // empty cluster found
                            ...
                            if (fat32)
                                fat_buffer.fat32[buffer_index] = 0x0FFFFFFF; // FAT32 EOC
                            else
                                fat_buffer.fat16[buffer_index] = 0xFFFF; // FAT16 EOC

                            // store FAT sector
                            if (!sd_write_sector(fat_start + fat_index, (unsigned char*)&fat_buffer))
                            {
                            ...
                            {
                                if (!sd_write_sector(fat_start + (i * fat_size) + fat_index, (unsigned char*)&fat_buffer))
                                {
                            ...
                            // initialize direntry
                            mmemset((void*)pEntry, 0, sizeof(DIRENTRY));
                            memcpy((void*)pEntry->Name, file->name, 11);
                            pEntry->Attributes = file->attributes;
                            pEntry->CreateDate = FILEDATE(2009, 9, 1);
                            pEntry->CreateTime = FILETIME(0, 0, 0);
                            pEntry->AccessDate = FILEDATE(2009, 9, 1);
                            pEntry->ModifyDate = FILEDATE(2009, 9, 1);
                            pEntry->ModifyTime = FILETIME(0, 0, 0);
                            pEntry->StartCluster = (unsigned short)cluster;
                            pEntry->HighCluster = fat32 ? (unsigned short)(cluster >> 16) : 0;
                            pEntry->FileSize = file->size;

                            // store dir entry
                            if (!sd_write_sector(iDirectorySector - 1, sector_buffer))
                            {
...
int UpdateEntry(fileTYPE *file)
{
    DIRENTRY *pEntry;

    if (!sd_read_sector(file->entry.sector, sector_buffer))
    {
    ...
    if ((pEntry->FileSize + cluster_size - 1) / (cluster_size << 9) != (file->size + cluster_size - 1) / (cluster_size << 9))
    {
        //printf("UpdateEntry(): different number of clusters!\r");
        //printf("pEntry->FileSize = %lu\r", pEntry->FileSize);
    ...
    if (!sd_write_sector(file->entry.sector, sector_buffer))
    {
