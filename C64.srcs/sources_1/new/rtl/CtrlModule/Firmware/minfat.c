...
int FileCreate(unsigned long iDirectory, fileTYPE *file)
{
    ...
    */
    ...
    {
        iDirectoryCluster = root_directory_cluster;		//0x2
        iDirectorySector = root_directory_start;		//0x2800
                ...
                while (fat_index < fat_size)
                {
                    ...
                    { // search through all entries in current sector
                        ...
                        {   // empty cluster found
                            ...
                            copyname((void*)pEntry->Name, file->name, 11);
                            ...
                            pEntry->StartCluster = (unsigned short)(((cluster & 0xff00) >> 8) | ((cluster & 0xff) << 8));					
                            pEntry->HighCluster = fat32 ? (unsigned short)(((cluster & 0xff0000) >> 8) | ((cluster & 0xff000000) >> 24)) : 0;
                            ...
                            if (!sd_write_sector(iDirectorySector - 1, sector_buffer))
                            {
                                ...
                                return(0);//iDirectorySector - 1);
                            }

                            file->start_cluster = cluster;
                            file->cluster = cluster;
                            //file->sector = 0;
                            file->sector = iDirectorySector - 1;
                            file->index = iEntry & 0x0F;
