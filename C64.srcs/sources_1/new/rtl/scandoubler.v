...
always @(posedge clk_x2) begin
   ...
   if(!scanline || scanlines == 2'b00) begin
	...
	2'b01: begin // reduce 25% = 1/2 + 1/4
	   r_out <= { 1'b0, sd_out[8:7] } + { 2'b00, sd_out[8] };
	   g_out <= { 1'b0, sd_out[5:4] }  + { 2'b00, sd_out[5]   };
	   b_out <= { 1'b0, sd_out[2:1] }   + { 2'b00, sd_out[2]   };
	end
	...
	2'b11: begin // reduce 75% = 1/4
	   r_out <= { 2'b00, sd_out[8] };
	   g_out <= { 2'b00, sd_out[5] };
	   b_out <= { 2'b00, sd_out[2] };
	end
      endcase
...
(* ramstyle = "no_rw_check" *) reg [8:0] sd_buffer [2047:0];
...
always @(negedge clk_pix) begin
   ...
   // rising edge of hblank indicates start of line
   if(hsD && !hs_in) begin
   //if(!hsDb && hb_in) begin
      hs_max <= hcnt;
      hcnt <= 10'd0;
...
always @(posedge clk_x2) begin
   ...
   //if(sd_hcnt == hs_rise-38) hs_sd <= 1'b1;
   if(sd_hcnt == hs_rise) hs_sd <= 1'b1;
   
   if(sd_hcnt == hb_fall)  hb_sd <= 1'b0;
   if(sd_hcnt == hb_rise) hb_sd <= 1'b1;
